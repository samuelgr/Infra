...
namespace _Infra_DebugTestInternal
{
  /// Type that should be thrown for debug assertions that are expected in tests. Simply contains
  /// a message containing the string that would have been triggered as a full-blown debug
  /// assertion. Intended for use in tests.
  class Assertion
  {
  public:
...
#ifdef _DEBUG
    /// Debug assertion expectation counter. Incremented whenever an object is created,
    /// decremented whenever an object is destroyed. If non-zero, debug assertions are expected
    /// and should be thrown as exceptions rather than used to trigger a full-blown application
...
namespace Infra
{
  ...
  using DebugAssertionException = _Infra_DebugTestInternal::Assertion;
} // namespace Infra
...
#define DebugAssert(expr, msg)                                                                     \
  do                                                                                               \
  {                                                                                                \
    if (::_Infra_DebugTestInternal::ExpectedAssertionContext::                                     \
            ShouldThrowAssertionFailureAsException())                                              \
    {                                                                                              \
      if (!(expr))                                                                                 \
        throw ::Infra::DebugAssertionException(                                                    \
...
#define ScopedExpectDebugAssertion()                                                               \
  [[maybe_unused]] ::_Infra_DebugTestInternal::ExpectedAssertionContext                            \
      __expectedDebugAssertionContext
